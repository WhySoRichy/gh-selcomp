<?php
/**
 * API para activar/desactivar autenticación 2FA - Administrador
...
if (!$activar && ($_SESSION['usuario_rol'] === 'admin' || $_SESSION['usuario_rol'] === 'administrador')) {
...
try {
    ...
    try {
        $detalles_log = $activar ? 'Verificación 2FA activada (Admin)' : 'Verificación 2FA desactivada (Admin)';
    ...
    if ($activar) {
        try {
            ...
            <p>La verificación en 2 pasos ha sido <strong style="color: #eb0045;">activada</strong> en tu cuenta de administrador del Portal Gestión Humana.</p>
            ...
            <div style="background: #fce4ec; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #eb0045;">
                ...
                <p style="margin: 10px 0 0 0;">La verificación en 2 pasos protege tu cuenta de administrador incluso si alguien conoce tu contraseña.</p>
            </div>
            <p style="color: #666;">Si no realizaste este cambio, contacta inmediatamente al equipo de TI.</p>
            <br>
            ...
            ';
            $mail->AltBody = "La verificación en 2 pasos ha sido activada en tu cuenta de administrador.";
            $mail->send();
        } catch (Exception $e) {
            error_log("Error al enviar confirmación 2FA admin: " . $e->getMessage());
        }
    }
